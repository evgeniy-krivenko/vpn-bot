...
func (l *logrusLogger) WithContextReqId(ctx context.Context) Logger {
	...
	return &logrusLogEntry{
		entry: l.logger.WithField("req-id", reqId),
	}
}
...
func (l *logrusLogEntry) WithField(key string, value interface{}) Logger {
	return &logrusLogEntry{entry: l.entry.WithField(key, value)}
}

func (l *logrusLogEntry) WithContextReqId(ctx context.Context) Logger {
	reqId, ok := ctx.Value("req-id").(string)
	if !ok {
		reqId = "none"
	}

	return &logrusLogEntry{entry: l.entry.WithField("req-id", reqId)}
}
